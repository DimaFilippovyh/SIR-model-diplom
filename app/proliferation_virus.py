...
def proliferation_virus_for_one_country_without_two_wave(country: Country):
    ...
    """
    name = country.name
    infected_people, none_infected_people, recovered_people = country.get_situation_in_country()
    ...
    tomr_infected_people = tmp_infected_people - tmp_recovered_people + infected_people[-1]
    tomr_none_infected_people = none_infected_people[-1] - tmp_infected_people
    tomr_recovered_people = recovered_people[-1] + tmp_recovered_people

    if (summ := tomr_infected_people + tomr_none_infected_people + tomr_recovered_people) > 1.1:
        raise Exception(f"Сумма превысила 1 = {summ}")

    # new_values = Country.normalize_solution([tomr_infected_people, tomr_none_infected_people, tomr_recovered_people])
    new_values = [tomr_infected_people, tomr_none_infected_people, tomr_recovered_people]
...
def proliferation_virus_for_one_country(country: Country):
    ...
    if len(recovered_people) > 92:
        tmp_two_wave_recovered_people = k_two_wave * (recovered_people[-1 - 90] - recovered_people[-2 - 90])
        # tmp_two_wave_recovered_people = k_two_wave * recovered_people[-1 - 90]
    else:
        tmp_two_wave_recovered_people = 0

    tomr_infected_people = infected_people[-1] - tmp_recovered_people + tmp_infected_people
    tomr_none_infected_people = none_infected_people[-1] - tmp_infected_people + tmp_two_wave_recovered_people
    tomr_recovered_people = recovered_people[-1] + tmp_recovered_people - tmp_two_wave_recovered_people
...
def proliferation_virus_with_two_wave_koef(country: Country, k_two_wave):
    """
    contacting - частота встречи людей
    recovered - коэф. выздоровления
    length_vir - протяженность болезни
...
def proliferation_virus(country: Country):
    ...
    if (summ := tomr_infected_people + tomr_none_infected_people + tomr_recovered_people) > 1.1:
        print(tomr_infected_people, tomr_none_infected_people, tomr_recovered_people)
        raise Exception(f"Сумма превысила 1 = {summ}")
...
def proliferation_virus_with_other_county(country: Country,
    ...
    infected_people_from_other_countries = [infected_people_in_countries.get(i) * country.contacts_country.get(i)
